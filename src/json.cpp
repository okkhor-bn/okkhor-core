...
public:
    ...
    Value parse_value() {
        ...
        switch (c) {
            ...
            case '"': { Value v; v.v = parse_string(); return v; }
            case 't': expect_word("true");  { Value v; v.v = true;  return v; }
            case 'f': expect_word("false"); { Value v; v.v = false; return v; }
            case 'n': expect_word("null");  { Value v; v.v = nullptr; return v; }
...
private:
    ...
    void expect(char c) {
        if (at_end() || get() != c) fail(std::string("expected '") + c + "'");
    }
    void expect_word(const char* w) {
        for (const char* p = w; *p; ++p) {
            if (at_end() || get() != *p) fail(std::string("expected literal ") + w);
        }
    }
    ...
    Value parse_object() {
        ...
        Object o;
        skip_ws();
        if (!at_end() && peek() == '}') { ++i_; Value v; v.v = std::move(o); return v; }
        while (true) {
            skip_ws();
    ...
    Value parse_array() {
        ...
        Array a;
        skip_ws();
        if (!at_end() && peek() == ']') { ++i_; Value v; v.v = std::move(a); return v; }
        while (true) {
        ...
        Value v; v.v = std::move(a); return v;
    }
    ...
    Value parse_number() {
        std::size_t start = i_;
        if (!at_end() && (peek() == '-' || peek() == '+')) ++i_;
        bool digits = false;
        while (!at_end()) {
            char c = peek();
            if ((c >= '0' && c <= '9')) { digits = true; ++i_; }
            else if (c == '.' || c == 'e' || c == 'E' || c == '-' || c == '+') ++i_;
            else break;
        }
...
Value parse(const std::string& text) {
    ...
    r.skip_ws();
    if (!r.at_end()) throw std::runtime_error("JSON parse error: trailing content");
    return v;
}
