...
template <typename T> void place(std::vector<T> &table, int id, T entry) {
  if (id < 0)
    throw std::runtime_error("negative id in mapping data");
  if (static_cast<std::size_t>(id) >= table.size())
    table.resize(static_cast<std::size_t>(id) + 1);
...
Mapping Mapping::load(const std::string &data_dir) {
  ...
  // ---- punctuation.json ----
  {
    ...
    for (const auto &[key, out] : doc.as_object())
      m.add_rule(key, Rule{TokenType::Punctuation, -1,
