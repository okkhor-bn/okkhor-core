            ...
            [&](const auto& u) {
                ...
                if constexpr (std::is_same_v<T, okkhor::OrthographicUnit>) {
                    ...
                    for (const auto& dc : u.conjuncts)
                        std::cout << " +dcons(" << engine.mapping().consonant(dc.value.id).latin << ")";
                    if (u.vowel)
                        std::cout << " +dvowel(" << engine.mapping().vowel(u.vowel->value.id).latin << ")";
                    for (const auto& a : u.accents)
                        std::cout << " +accent(" << engine.mapping().accent(a.id).latin << ")";
                    if (u.explicit_hasanta) std::cout << " +hasanta";
                    if (u.zwnj_after) std::cout << " +zwnj";
                    std::cout << "]";
...
int main(int argc, char** argv) {
    ...
    try {
        okkhor::Engine engine = okkhor::Engine::from_data_dir(okkhor::find_data_dir(data_dir));
        ...
        if (!words.empty()) {
            ...
            if (verbose) describe(engine, joined);
            else std::cout << engine.transliterate(joined) << "\n";
            return 0;
        }

        std::string line;
        while (std::getline(std::cin, line)) {
            if (verbose) describe(engine, line);
            else std::cout << engine.transliterate(line) << "\n";
        }
        return 0;
